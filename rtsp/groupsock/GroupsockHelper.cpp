...
void TraceMsgA2(LPCSTR pFormat, ...)
{
	va_list args;
...
static int createSocket(int type)
{
    ...
    if (SOCK_STREAM == type)
    {
        ...
        tv.tv_sec = 0; //2s
        tv.tv_usec = 500*1000;
        setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, (char *)&tv, sizeof(tv));  //add by lulanhua 2018.10.17
        setsockopt(sock, SOL_SOCKET, SO_SNDTIMEO, (char *)&tv, sizeof(tv));  //add by lulanhua 2018.10.17
    }
    return sock;
