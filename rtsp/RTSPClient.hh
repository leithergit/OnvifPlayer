...
public:
    ...
    void SetConnnectTimeout(int nTimeout = 5000)
    {
        if (nTimeout <= 0)
            nConnectTimeout = 5000;
        else
            nConnectTimeout = nTimeout;
    }
...
private:
    ...
    public:
        ...
        void enqueue(RequestRecordPtr request); // "request" must not be NULL
        RequestRecordPtr dequeue();
        void putAtHead(RequestRecordPtr request); // "request" must not be NULL
        RequestRecordPtr findByCSeq(unsigned cseq);
    ...
    private:
        RequestRecordPtr fHead;
        RequestRecordPtr fTail;
    };
    ...
    unsigned sendRequest(RequestRecordPtr request);
    void handleRequestError(RequestRecordPtr request);
    ...
    Boolean resendCommand(RequestRecordPtr request);
...
private:
    ...
    int	nConnectTimeout;
    // timeout of connect to server in millionsecond;
