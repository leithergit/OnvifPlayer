...
//StackWalker ShowStack(0x3F,"E:\\Symbols");
...
unsigned RTSPClient::sendDescribeCommand(responseHandler* responseHandler, Authenticator* authenticator)
{
    ...
    return sendRequest(make_shared<RequestRecord>(++fCSeq, "DESCRIBE", responseHandler));
}

unsigned RTSPClient::sendOptionsCommand(responseHandler* responseHandler, Authenticator* authenticator)
{
    if (authenticator != NULL) fCurrentAuthenticator = *authenticator;
    return sendRequest(make_shared< RequestRecord>(++fCSeq, "OPTIONS", responseHandler));
}
...
{
    if (authenticator != NULL) fCurrentAuthenticator = *authenticator;
	return sendRequest(make_shared< RequestRecord>(++fCSeq, "ANNOUNCE", responseHandler, nullptr, nullptr, False, 0.0f, 0.0f, 0.0f, sdpDescription));
}
                                      ...
                                      Authenticator* authenticator)
{
	...
	return sendRequest(make_shared< RequestRecord>(++fCSeq, "SETUP", responseHandler, nullptr, &subsession, booleanFlags));
}
...
{
    if (authenticator != NULL) fCurrentAuthenticator = *authenticator;
	return sendRequest(make_shared< RequestRecord>(++fCSeq, "PLAY", responseHandler, &session, nullptr, 0, start, end, scale));
}
...
{
    if (authenticator != NULL) fCurrentAuthenticator = *authenticator;
	return sendRequest(make_shared< RequestRecord>(++fCSeq, "PLAY", responseHandler, nullptr, &subsession, 0, start, end, scale));
}

unsigned RTSPClient::sendPauseCommand(MediaSession& session, responseHandler* responseHandler, Authenticator* authenticator)
{
    if (authenticator != NULL) fCurrentAuthenticator = *authenticator;
	return sendRequest(make_shared< RequestRecord>(++fCSeq, "PAUSE", responseHandler, &session));
}
...
{
    if (authenticator != NULL) fCurrentAuthenticator = *authenticator;
	return sendRequest(make_shared< RequestRecord>(++fCSeq, "PAUSE", responseHandler, nullptr, &subsession));
}

unsigned RTSPClient::sendRecordCommand(MediaSession& session, responseHandler* responseHandler, Authenticator* authenticator)
{
    if (authenticator != NULL) fCurrentAuthenticator = *authenticator;
	return sendRequest(make_shared< RequestRecord>(++fCSeq, "RECORD", responseHandler, &session));
}
...
{
    if (authenticator != NULL) fCurrentAuthenticator = *authenticator;
	return sendRequest(make_shared< RequestRecord>(++fCSeq, "RECORD", responseHandler, nullptr, &subsession));
}

unsigned RTSPClient::sendTeardownCommand(MediaSession& session, responseHandler* responseHandler, Authenticator* authenticator)
{
    if (authenticator != NULL) fCurrentAuthenticator = *authenticator;
	return sendRequest(make_shared< RequestRecord>(++fCSeq, "TEARDOWN", responseHandler, &session));
}
...
{
    if (authenticator != NULL) fCurrentAuthenticator = *authenticator;
	return sendRequest(make_shared< RequestRecord>(++fCSeq, "TEARDOWN", responseHandler, nullptr, &subsession));
}
        ...
        Authenticator* authenticator)
{
	...
	unsigned result = sendRequest(make_shared< RequestRecord>(++fCSeq, "SET_PARAMETER", responseHandler, &session, nullptr, False, 0.0f, 0.0f, 0.0f, paramString));
    delete[] paramString;
    return result;
}
        ...
        Authenticator* authenticator)
{
	...
	unsigned result = sendRequest(make_shared< RequestRecord>(++fCSeq, "GET_PARAMETER", responseHandler, &session, nullptr, False, 0.0f, 0.0f, 0.0f, paramString));
    delete[] paramString;
    return result;
}
      ...
      fSessionCookieCounter(0), fHTTPTunnelingConnectionIsPending(False)
{
    ...
    char const* const libName = "Mago Streaming Media v";
...
int RTSPClient::connectToServer(int socketNum, portNumBits remotePortNum)
{
    ...
    if (connect(socketNum, (struct sockaddr*) &remoteName, sizeof remoteName) != 0)
    {
        ...
        fd_set FDConnect;
        FD_ZERO(&FDConnect);
        FD_SET(socketNum, &FDConnect);
        connecttimeout.tv_sec = nConnectTimeout/1000;
        connecttimeout.tv_usec = (nConnectTimeout % 1000) * 1000;	//连接超时200 ms,tv_usec的时间单位是微秒

        int nRes = select(0, 0, &FDConnect, 0, &connecttimeout);
        if (!nRes ||SOCKET_ERROR == nRes)
        {
...
unsigned RTSPClient::sendRequest(RequestRecordPtr request)
{
    char* cmd = NULL;
    do
    {
        ...
        else if (strcmp(request->commandName(), "SETUP") == 0)
        {
            ...
            extraHeaders = new char[transportSize + strlen(sessionStr)+20];
            extraHeadersWereAllocated = True;
            if(streamUsingTCP)
                sprintf(extraHeaders, "Blocksize: 65535\r\n%s%s", transportStr, sessionStr);
            else
...
void RTSPClient::handleRequestError(RequestRecordPtr request)
{
	...
	{
		char *pszResultstring = strDup(envir().getResultMsg());
		(*request->handler())(this, resultCode, pszResultstring);
...
Boolean RTSPClient::resendCommand(RequestRecordPtr request)
{
...
Boolean RTSPClient::setupHTTPTunneling1()
{
	...
	return sendRequest(make_shared< RequestRecord>(1, "GET", responseHandlerForHTTP_GET)) != 0;
}
...
Boolean RTSPClient::setupHTTPTunneling2()
{
	...
	return sendRequest(make_shared< RequestRecord>(1, "POST", nullptr)) != 0;
}
...
void RTSPClient::handleResponseBytes(int newBytesRead)
{
    ...
    do
    {
        ...
        RequestRecordPtr foundRequest = NULL;
            ...
            if (foundRequest != NULL)
            {
                ...
                else if (responseCode == 401 && handleAuthenticationFailure(wwwAuthenticateParamsStr))
                {
                    ...
                    if (strcmp(foundRequest->commandName(), "GET") == 0)
                    {
                        ...
                        // because some servers close the original connection after returning the "continue Unauthorized".
...
//	printf("%s delete %8x.\n", __FUNCTION__, this);
// 	TraceMsgA2("**************************************\n");
// 	TraceMsgA2("%s delete %8x.\n", __FUNCTION__, this);
// 	ShowStack.ShowCallstack();
// 	TraceMsgA2("**************************************\n");
   // delete fNext;
...
void RTSPClient::RequestQueue::enqueue(RequestRecordPtr request)
{
    if (fTail == NULL)
    {
        fHead = request;
...
RTSPClient::RequestRecordPtr RTSPClient::RequestQueue::dequeue()
{
    RequestRecordPtr request = fHead;
    if (fHead == fTail)
    {
        fHead = NULL;
        fTail = NULL;
...
void RTSPClient::RequestQueue::putAtHead(RequestRecordPtr request)
{
    request->next() = fHead;
    fHead = request;
    if (fTail == NULL)
...
RTSPClient::RequestRecordPtr RTSPClient::RequestQueue::findByCSeq(unsigned cseq)
{
    RequestRecordPtr request;
