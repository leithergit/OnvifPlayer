...
// #else
    ...
    sprintf(CNAME, "RTCP host %d", (unsigned)(our_random()*0x7FFFFFFF));
//#endif
...
MediaSubsession::MediaSubsession(MediaSession& parent)
    : serverPortNum(0), sink(NULL), miscPtr(NULL),
      ...
      fSessionId(NULL),_width(1280),_height(720),_spsLen(0),_ppsLen(0)
{
    rtpInfo.seqNum = 0;
    rtpInfo.timestamp = 0;
    ...
    do
    {
        ...
        while (*line != '\0' && *line != '\r' && *line != '\n')
        {
            ...
            else if (sscanf(sdpLine, " sprop-parameter-sets = %[^; \t\r\n]", valueStr) == 1)
            {
                ...
                if(strlen(fSpropParameterSets)>0)
                {
                    std::string strRes = fSpropParameterSets;
                    size_t pos_sps_end = strRes.find(',');
                    std::string str_base64;
                    str_base64.assign(strRes, 0, pos_sps_end);
#ifdef SVAC
                    char* pSPS = _frameHeader+24;
#else
                    char* pSPS = _frameHeader+16;
#endif
                    pSPS[0] = pSPS[1] = pSPS[2] = 0;
                    pSPS[3] = 1;

                    _spsLen = ez_base64decode(pSPS+4, str_base64.c_str());
                    _spsLen += 4;

                    size_t pps_start = (pos_sps_end+strlen(","));
                    size_t pos_pps_end = strRes.find("\r\n", 0);
                    str_base64.assign(strRes, pps_start, pos_pps_end-pps_start);
#ifdef SVAC
                    char* pPPS = _frameHeader+24+_spsLen;
#else
                    char* pPPS = _frameHeader+16+_spsLen;
#endif
                    pPPS[0] = pPPS[1] = pPPS[2] = 0;
                    pPPS[3] = 1;

                    _ppsLen = ez_base64decode(pPPS+4, str_base64.c_str());
                    _ppsLen += 4;

                    int ret = decsps((unsigned char*)pSPS, _spsLen, &_width, &_height);
                }
            ...
            {
                delete[] fEmphasis;
                fEmphasis = strDup(valueStr);
            }
            else if (sscanf(sdpLine, " channel-order = %[^; \t\r\n]", valueStr) == 1)
            {
                // Note: We used "sdpLine" here, because the value is case-sensitive.
                delete[] fChannelOrder;
                fChannelOrder = strDup(valueStr);
            }
            else
            {
