...
{ if (!soap_tag_cmp(key, "Host"))
  ...
  else if (!soap_tag_cmp(key, "WWW-Authenticate") || !soap_tag_cmp(key, "Proxy-Authenticate"))
  {
    ...
    if (!soap_tag_cmp(val, "NTLM*"))
	  ...
	  soap->nonce = soap_strdup(soap, soap_get_header_attribute(soap, val + 6, "nonce"));
	  soap->alg = soap_strdup(soap, soap_get_header_attribute(soap, val + 6, "algorithm"));
	  soap->qop = soap_strdup(soap, soap_get_header_attribute(soap, val + 6, "qop"));
	  soap->ncount = soap_strdup(soap, soap_get_header_attribute(soap, val + 6, "nc"));
	  soap->cnonce = soap_strdup(soap, soap_get_header_attribute(soap, val + 6, "cnonce"));
	  soap->response = soap_strdup(soap, soap_get_header_attribute(soap, val + 6, "response"));
	  soap->opaque = soap_strdup(soap, soap_get_header_attribute(soap, val + 6, "opaque"));
