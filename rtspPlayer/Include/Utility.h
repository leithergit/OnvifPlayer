...
#ifdef _DEBUG
#ifdef _UNICODE
#define TraceFunction()	CFunctionTimeSpan Tx(__FUNCTIONW__);
#define TraceFileLine()	CFunctionTimeSpan TFL(__FILEW__,__LINE__);
#else
#define TraceFunction()	CFunctionTimeSpan Tx(__FUNCTION__);
#define TraceFileLine()	CFunctionTimeSpan TFL(__FILE__,__LINE__);
#endif
class CFunctionTimeSpan
{
	explicit CFunctionTimeSpan() {};
public:
	CFunctionTimeSpan(TCHAR* szFunction)
	{
		ZeroMemory(this, sizeof(CFunctionTimeSpan));
		_tcscpy_s(m_szFunction, 1024, szFunction);
		m_nTimeIn = timeGetTime();
	}
	CFunctionTimeSpan(TCHAR* FileName, int nLine)
	{
		ZeroMemory(this, sizeof(CFunctionTimeSpan));
		_tcscpy_s(m_szFunction, 1024, FileName);
		m_FileLine = nLine;
		m_nTimeIn = timeGetTime();
	}
	~CFunctionTimeSpan()
	{
		TCHAR szText[1024] = { 0 };
		if (!m_FileLine)
			_stprintf_s(szText, 1024, _T("%s\tTimeSpan = %d ms.\n"), m_szFunction, (timeGetTime() - m_nTimeIn));
		else
			_stprintf_s(szText, 1024, _T("%s:%d\tTimeSpan = %d ms.\n"), m_szFunction, m_FileLine, (timeGetTime() - m_nTimeIn));
		OutputDebugString(szText);
	}
private:
	DWORD	m_nTimeIn;
	TCHAR	m_szFunction[1024];
	int		m_FileLine = 0;
};
#else 
#define TraceFunction()	
#define TraceFileLine()
